"""Panel border chrome and footer styling for `MonitorApp` (alloy-o89.1).

Behavioral/id-based checks only: widget ids, border titles, loaded stylesheet,
palette tokens in TCSS, and footer key chip text. Does not assert geometry or
...
def _border_is_heavy(widget: Static | DataTable) -> bool:
    border = widget.styles.border
    return all(getattr(border, edge)[0] == "heavy" for edge in ("top", "right", "bottom", "left"))
...
async def test_each_panel_widget_has_a_heavy_border_after_mount(panel_id: str):
    limits = _claude_limits(42.0)
    snapshot = _snapshot(limits=limits, runs=[_run("run-1")])
    app = MonitorApp(
        snapshot_source=lambda: snapshot,
    ...
    async with app.run_test() as pilot:
        await pilot.pause()
        widget = _panel_widget(app, panel_id)
        assert _border_is_heavy(widget)
